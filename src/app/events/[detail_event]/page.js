...
export default async function Page({ params }) {
  ...
  return (
    <main>
      <div
        key={detailEvent.data.events.id}
        className="flex flex-row justify-items-center p-4 w-3/4 gap-8 m-auto self-center justify-center"
      >
        <div className="flex flex-col gap-8">
          <FormModal idEvent={idEvent} />
          <div className="w-[500px]">
            <Image
              src={detailEvent.data.events.image}
              width={600}
              height={300}
              alt="Image Event"
          ...
          <div className=" card shadow shadow-slate-950 border-slate-950 w-[500px] p-3">
            ...
            <p className=" text-justify">
              {detailEvent.data.events.description}
            </p>
          </div>
        </div>
